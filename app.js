...
function applyTheme(theme){
  ...
  const button=document.querySelector('#theme-toggle');button.querySelector('span').textContent=theme==='contrast'?'☀️':'🌙';button.setAttribute('aria-label',theme==='contrast'?'Activer le mode clair':'Activer le mode sombre');
}
...
function updateSoundButton(){const button=document.querySelector('#sound-toggle');button.querySelector('span').textContent=soundEnabled?'🔔':'🔕';button.title=soundEnabled?'Couper les alertes sonores':'Activer les alertes sonores';button.setAttribute('aria-label',button.title);button.classList.toggle('active',soundEnabled)}
...
function setAppMode(next){
  ...
  renderCart(); toast(appMode==='tab'?'Mode ardoise partagé':appMode==='order'?'Mode commande simple':appMode==='expenses'?'Suivi des dépenses':appMode==='raffle'?'Tombola partagée':'Classement des jeux');
}
...
async function refresh() {
  ...
  const [balanceResult, logsResult, expensesResult,raffleSettingsResult,raffleEntriesResult,teamStatsResult,gameSettingsResult,gameScoresResult,gameLogsResult,gameWinnersResult] = await Promise.all([
...
if(db){ db.channel('mojito-inn-live').on('postgres_changes',{event:'*',schema:'public',table:'ardoise_state'},refresh).on('postgres_changes',{event:'INSERT',schema:'public',table:'ardoise_logs'},payload=>{showRemoteAlert(payload.new);refresh()}).on('postgres_changes',{event:'UPDATE',schema:'public',table:'ardoise_logs'},refresh).on('postgres_changes',{event:'DELETE',schema:'public',table:'ardoise_logs'},refresh).on('postgres_changes',{event:'*',schema:'public',table:'depenses'},refresh).on('postgres_changes',{event:'*',schema:'public',table:'tombola_settings'},refresh).on('postgres_changes',{event:'*',schema:'public',table:'tombola_entries'},refresh).on('postgres_changes',{event:'*',schema:'public',table:'stats_equipe'},refresh).on('postgres_changes',{event:'*',schema:'public',table:'game_settings'},refresh).on('postgres_changes',{event:'*',schema:'public',table:'game_scores'},refresh).on('postgres_changes',{event:'*',schema:'public',table:'game_point_logs'},refresh).on('postgres_changes',{event:'*',schema:'public',table:'game_winners'},refresh).subscribe(status=>{if(status==='SUBSCRIBED')syncStatus('Ardoise partagée · en direct','online')}); }
updateMenuPrices();applyTheme(localStorage.getItem(THEME_KEY)||'sand');updateSoundButton();render(); renderCart();renderExpenses();renderOrderLogs();renderRaffle();renderCitizenSales();renderTeamStats();renderGame();renderPreviousWinners(); refresh(); if(!operatorName)setTimeout(openName,250);
