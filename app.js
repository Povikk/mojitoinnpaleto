...
function renderPreviousWinners(){document.querySelector('#previous-winners').innerHTML=game.winnersError?'<div class="team-empty winners-error">Historique non installé dans Supabase</div>':game.winners.length?game.winners.map(game=>`<div class="previous-winner"><small>Partie</small><b>🏆 ${escapeHTML(game.title)}</b><ol><li><em>1</em><span>${escapeHTML(game.first.name)}</span><strong>${game.first.points} pt</strong></li>${game.second.name?`<li><em>2</em><span>${escapeHTML(game.second.name)}</span><strong>${game.second.points} pt</strong></li>`:''}${game.third.name?`<li><em>3</em><span>${escapeHTML(game.third.name)}</span><strong>${game.third.points} pt</strong></li>`:''}</ol></div>`).join(''):'<div class="team-empty">Aucune partie terminée avec des points</div>'}
