...
function renderGame(){const sorted=[...game.scores].sort((a,b)=>b.points-a.points||a.name.localeCompare(b.name,'fr'));document.querySelector('#game-title').value=game.title==='Nouvelle partie'?'':game.title;document.querySelector('#game-type').value=game.type;document.querySelector('#game-heading').textContent=game.title||'Classement';document.querySelector('#game-player-count').textContent=sorted.length;document.querySelector('#game-total-points').textContent=sorted.reduce((sum,x)=>sum+x.points,0);document.querySelector('#game-leader').textContent=sorted.length&&sorted[0].points>0?sorted.filter(x=>x.points===sorted[0].points).map(x=>x.name).join(', '):'—';document.querySelector('#leaderboard').innerHTML=sorted.length?sorted.map(player=>{const rank=sorted.findIndex(x=>x.points===player.points)+1;return `<li><span class="rank">${rank}</span><span class="leader-info"><b>${escapeHTML(player.name)}</b><small>${player.points} point${player.points>1?'s':''}</small></span><span class="score-controls"><button data-score-id="${player.id}" data-delta="-1" type="button">−1</button><strong>${player.points}</strong><button data-score-id="${player.id}" data-delta="1" type="button">+1</button><button class="plus-two" data-score-id="${player.id}" data-delta="2" type="button">+2</button><button class="delete-player" data-delete-score="${player.id}" type="button">×</button></span></li>`}).join(''):'<li class="empty">Ajoute un nom pour commencer 🎵</li>';document.querySelector('#game-log-count').textContent=`${game.logs.length} attribution${game.logs.length>1?'s':''}`;document.querySelector('#game-log').innerHTML=game.logs.length?game.logs.slice(0,50).map(log=>`<li><span class="history-icon ${log.delta<0?'add':''}">${log.delta>0?'+':'−'}</span><span class="history-info"><b>${escapeHTML(log.player)}</b><small>${escapeHTML(log.operator)} · ${new Date(log.date).toLocaleTimeString('fr-FR',{hour:'2-digit',minute:'2-digit'})}</small></span><span class="history-amount ${log.delta<0?'add':''}">${log.delta>0?'+':''}${log.delta}</span></li>`).join(''):'<li class="empty">Aucun point attribué</li>'}
