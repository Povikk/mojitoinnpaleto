...
function renderGifts(){
  const money=gifts.filter(x=>x.type==='money').reduce((sum,x)=>sum+x.amount,0),drinks=gifts.filter(x=>x.type==='drink').reduce((sum,x)=>sum+x.remaining,0),people=new Set(gifts.filter(x=>x.type==='money'||x.remaining>0).map(x=>x.name.toLowerCase())).size,admin=isAdminName(operatorName);
  document.querySelector('#gift-money-total').textContent=format(money);document.querySelector('#gift-drinks-left').textContent=drinks;document.querySelector('#gift-people-count').textContent=people;document.querySelector('#gift-count').textContent=`${gifts.length} cadeau${gifts.length>1?'x':''}`;document.querySelector('#reset-gifts').hidden=!admin;
  document.querySelector('#gift-list').innerHTML=gifts.length?gifts.map(item=>`<article class="gift-item ${item.type}"><div class="gift-icon">${item.type==='drink'?'🍹':'💵'}</div><div class="gift-info"><b>${escapeHTML(item.name)}</b><small>${item.type==='drink'?`${item.remaining} restante${item.remaining>1?'s':''} sur ${item.quantity}`:`${format(item.amount)} offerts`}${item.note?' · '+escapeHTML(item.note):''}</small><em>Par ${escapeHTML(item.operator)} · ${new Date(item.date).toLocaleDateString('fr-FR')}</em></div><div class="gift-actions">${item.type==='drink'?`<button class="gift-use" data-gift-use="${item.id}" type="button" ${item.remaining<1?'disabled':''}>−1 conso</button>`:''}${admin?`<button class="gift-delete" data-gift-delete="${item.id}" type="button" title="Supprimer ce cadeau">×</button>`:''}</div></article>`).join(''):'<div class="team-empty">Aucun cadeau enregistré pour le moment</div>';
}
...
async function refresh() {
  ...
  const [balanceResult, logsResult, expensesResult,raffleSettingsResult,raffleEntriesResult,teamStatsResult,gameSettingsResult,gameScoresResult,gameLogsResult,gameWinnersResult,giftsResult] = await Promise.all([
    ...
    db.from('gifts').select('id,beneficiary,gift_type,quantity,remaining,amount,note,operator_name,created_at').order('created_at',{ascending:false}).limit(100)
  ]);
  ...
  gifts=giftsResult.error?[]:giftsResult.data.map(x=>({id:x.id,name:x.beneficiary,type:x.gift_type,quantity:Number(x.quantity),remaining:Number(x.remaining),amount:Number(x.amount),note:x.note,operator:x.operator_name,date:new Date(x.created_at).getTime()}));
...
document.querySelector('#gift-form').addEventListener('submit',async event=>{event.preventDefault();if(!requireName()||!db)return;const name=document.querySelector('#gift-name').value.trim(),type=document.querySelector('#gift-type').value,value=parseAmount(document.querySelector('#gift-value').value),note=document.querySelector('#gift-note').value.trim();if(!name||!Number.isFinite(value)||value<=0||type==='drink'&&!Number.isInteger(value))return toast('Vérifie le nom et la quantité');const{error}=await db.rpc('ajouter_cadeau',{p_beneficiary:name,p_type:type,p_value:value,p_note:note,p_operator_name:operatorName});if(error)return toast(`Cadeau non enregistré : ${error.message}`);event.target.reset();document.querySelector('#gift-value').value=1;document.querySelector('#gift-type').dispatchEvent(new Event('change'));await refresh();toast(`Cadeau ajouté pour ${name} 🎁`)});
document.querySelector('#gift-list').addEventListener('click',async event=>{const use=event.target.closest('[data-gift-use]'),remove=event.target.closest('[data-gift-delete]');if(use){if(!requireName()||!db)return;use.disabled=true;const{error}=await db.rpc('utiliser_conso_cadeau',{p_gift_id:Number(use.dataset.giftUse),p_operator_name:operatorName});if(error){use.disabled=false;return toast(error.message)}await refresh();toast('Une consommation gratuite utilisée ✓')}if(remove){if(!isAdminName(operatorName)||!db)return;const item=gifts.find(x=>String(x.id)===remove.dataset.giftDelete);if(!item||!confirm(`Supprimer le cadeau de ${item.name} ?`))return;const{error}=await db.rpc('supprimer_cadeau',{p_gift_id:item.id,p_requester_name:operatorName});if(error)return toast(error.message);await refresh();toast('Cadeau supprimé')}});
