...
document.querySelector('#add-previous-players').addEventListener('click',async()=>{if(!db)return toast('Branche d’abord la base Supabase');const button=document.querySelector('#add-previous-players');button.disabled=true;const{data,error}=await db.rpc('ajouter_participants_precedents');button.disabled=false;if(error)return toast(error.message);await refresh();toast(data>0?`${data} participant${data>1?'s':''} ajouté${data>1?'s':''} depuis la partie précédente`:'Tous les participants précédents sont déjà présents')});
