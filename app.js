...
function showRemoteAlert(row){if(!row||String(row.operator_name||'').trim().toLowerCase()===operatorName.trim().toLowerCase())return;const alert=document.querySelector('#remote-alert'),amount=Number(row.amount||0),quantity=Number(row.quantity||1);remoteAlertCount=Date.now()-Number(alert.dataset.last||0)<1800?remoteAlertCount+1:1;alert.dataset.last=Date.now();const message=remoteAlertCount>1?`${remoteAlertCount} nouvelles opérations sur l’ardoise`:row.operation==='add'?`${row.operator_name} a ajouté ${format(amount)} à l’ardoise`:`${row.operator_name} a déduit ${quantity>1?quantity+' × ':''}${row.item_name||format(amount)}`;alert.querySelector('b').textContent=message;alert.classList.add('show',row.operation==='add'?'addition':'payment');alert.classList.remove(row.operation==='add'?'payment':'addition');clearTimeout(showRemoteAlert.timer);showRemoteAlert.timer=setTimeout(()=>{alert.classList.remove('show');remoteAlertCount=0},5000);playRemoteSound()}
...
function setAppMode(next){
  ...
  renderCart(); toast(appMode==='tab'?'Mode ardoise partagé':appMode==='order'?'Mode commande simple':appMode==='expenses'?'Suivi des dépenses':appMode==='gifts'?'Suivi des cadeaux':appMode==='raffle'?'Tombola partagée':'Classement des jeux');
}
...
document.querySelector('#remote-alert').addEventListener('click',()=>{document.querySelector('#remote-alert').classList.remove('show');remoteAlertCount=0;setAppMode('tab');document.querySelector('.balance-card').scrollIntoView({behavior:'smooth',block:'start'})});

async function refresh() {
  if (!db) { syncStatus('Configuration Supabase requise', 'error'); render(); return; }
  const [balanceResult, logsResult, expensesResult,raffleSettingsResult,raffleEntriesResult,teamStatsResult,gameSettingsResult,gameScoresResult,gameLogsResult,gameWinnersResult,giftsResult] = await Promise.all([
    ...
    db.from('game_winners').select('game_title,winner_name,points,second_name,second_points,third_name,third_points,standings,created_at').order('created_at',{ascending:false}).limit(3),
  ...
  if(!gameSettingsResult.error&&!gameScoresResult.error&&!gameLogsResult.error){game={title:gameSettingsResult.data.title,type:gameSettingsResult.data.game_type,scores:gameScoresResult.data.map(x=>({id:x.id,name:x.name,points:Number(x.points)})),logs:gameLogsResult.data.map(x=>({player:x.player_name,delta:Number(x.delta),operator:x.operator_name,date:new Date(x.created_at).getTime()})),winners:gameWinnersResult.error?[]:gameWinnersResult.data.map(x=>({title:x.game_title,first:{name:x.winner_name,points:Number(x.points)},second:{name:x.second_name,points:Number(x.second_points||0)},third:{name:x.third_name,points:Number(x.third_points||0)},standings:Array.isArray(x.standings)?x.standings.map(player=>({name:player.name,points:Number(player.points)})):[],date:new Date(x.created_at).getTime()})),winnersError:Boolean(gameWinnersResult.error)}}
  syncStatus('Ardoise partagée · en direct','online'); render();renderExpenses();renderGifts();renderRaffle();renderTeamStats();renderOrderLogs();renderGame();renderPreviousWinners();
}
...
if(db){ db.channel('mojito-inn-live').on('postgres_changes',{event:'*',schema:'public',table:'ardoise_state'},refresh).on('postgres_changes',{event:'INSERT',schema:'public',table:'ardoise_logs'},payload=>{showRemoteAlert(payload.new);refresh()}).on('postgres_changes',{event:'UPDATE',schema:'public',table:'ardoise_logs'},refresh).on('postgres_changes',{event:'DELETE',schema:'public',table:'ardoise_logs'},refresh).on('postgres_changes',{event:'*',schema:'public',table:'depenses'},refresh).on('postgres_changes',{event:'*',schema:'public',table:'gifts'},refresh).on('postgres_changes',{event:'*',schema:'public',table:'tombola_settings'},refresh).on('postgres_changes',{event:'*',schema:'public',table:'tombola_entries'},refresh).on('postgres_changes',{event:'*',schema:'public',table:'stats_equipe'},refresh).on('postgres_changes',{event:'*',schema:'public',table:'game_settings'},refresh).on('postgres_changes',{event:'*',schema:'public',table:'game_scores'},refresh).on('postgres_changes',{event:'*',schema:'public',table:'game_point_logs'},refresh).on('postgres_changes',{event:'*',schema:'public',table:'game_winners'},refresh).subscribe(status=>{if(status==='SUBSCRIBED')syncStatus('Ardoise partagée · en direct','online')}); }
updateMenuPrices();applyTheme(localStorage.getItem(THEME_KEY)||'sand');updateSoundButton();render(); renderCart();renderExpenses();renderGifts();renderOrderLogs();renderRaffle();renderCitizenSales();renderTeamStats();renderGame();renderPreviousWinners(); refresh();openAccessGate();if((!db||sessionStorage.getItem(ACCESS_KEY)==='yes')&&!operatorName)setTimeout(openName,250);
