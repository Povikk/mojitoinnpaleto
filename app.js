...
let teamStats=[];
let game={title:'Nouvelle partie',type:'Blind Test',scores:[],logs:[],winners:[]};
...
function renderGame(){
  ...
  const shown=[...game.scores].sort(gameSort==='alpha'?(a,b)=>a.name.localeCompare(b.name,'fr',{sensitivity:'base'}):gameSort==='points'?(a,b)=>b.points-a.points||a.name.localeCompare(b.name,'fr'):(a,b)=>a.id-b.id);
  document.querySelector('#game-sort').value=gameSort;
  document.querySelector('#game-title').value=game.title==='Nouvelle partie'?'':game.title;document.querySelector('#game-type').value=game.type;document.querySelector('#game-heading').textContent=game.title||'Classement';document.querySelector('#game-player-count').textContent=shown.length;document.querySelector('#game-total-points').textContent=shown.reduce((sum,x)=>sum+x.points,0);document.querySelector('#game-leader').textContent=byPoints.length&&byPoints[0].points>0?byPoints.filter(x=>x.points===byPoints[0].points).map(x=>x.name).join(', '):'—';
  document.querySelector('#leaderboard').innerHTML=shown.length?shown.map((player,index)=>`<li><span class="rank">${index+1}</span><span class="leader-info"><b>${escapeHTML(player.name)}</b><small>${player.points} point${player.points>1?'s':''}</small></span><span class="score-controls"><button data-score-id="${player.id}" data-delta="-1" type="button">−1</button><strong>${player.points}</strong><button data-score-id="${player.id}" data-delta="1" type="button">+1</button><button class="plus-two" data-score-id="${player.id}" data-delta="2" type="button">+2</button><button class="delete-player" data-delete-score="${player.id}" type="button">×</button></span></li>`).join(''):'<li class="empty">Ajoute un nom pour commencer 🎵</li>';
...
function renderPreviousWinners(){document.querySelector('#previous-winners').innerHTML=game.winners.length?game.winners.map(winner=>`<div class="previous-winner"><span>🏆</span><b>${escapeHTML(winner.name)}</b><small>${escapeHTML(winner.title)} · ${winner.points} pt${winner.points>1?'s':''}</small></div>`).join(''):'<div class="team-empty">Aucune partie terminée pour le moment</div>'}
...
document.querySelector('#game-settings-form').addEventListener('submit',async event=>{event.preventDefault();if(!db)return toast('Branche d’abord la base Supabase');const title=document.querySelector('#game-title').value.trim()||'Nouvelle partie',type=document.querySelector('#game-type').value;const{error}=await db.rpc('configurer_jeu',{p_title:title,p_game_type:type});if(error)return toast(error.message);await refresh();toast('Partie enregistrée')});
...
document.querySelector('#reset-game').addEventListener('click',async()=>{if(!db||!confirm('Effacer le classement et commencer une nouvelle partie ?'))return;const{error}=await db.rpc('nouvelle_partie_jeu');if(error)return toast(error.message);await refresh();toast('Nouvelle partie prête')});
