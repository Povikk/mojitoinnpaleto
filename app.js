...
document.querySelector('#reset-game').addEventListener('click',async()=>{if(!db||!confirm('Finaliser et sauvegarder cette partie ? Le gagnant sera enregistré et le classement remis à zéro.'))return;const{error}=await db.rpc('nouvelle_partie_jeu');if(error)return toast(`Sauvegarde impossible : ${error.message}`);await refresh();toast('Partie sauvegardée · gagnant enregistré')});
