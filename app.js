...
document.querySelector('#reset-game').addEventListener('click',async()=>{if(!db||!confirm('Terminer cette partie, enregistrer le gagnant et commencer une nouvelle partie ?'))return;const{error}=await db.rpc('nouvelle_partie_jeu');if(error)return toast(`Nouvelle partie impossible : ${error.message}`);await refresh();toast('Gagnant enregistré · nouvelle partie prête')});
